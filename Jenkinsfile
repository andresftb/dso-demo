pipeline {
  environment {
    ...
    DEV_URL = 'http://34.68.245.93:30080/'
  }
  agent {
    kubernetes {
  ...
  stages {
    ...
    stage('Package') {
      parallel {
        ...
        stage('OCI Image BnP') {
          steps {
            container('kaniko') {
              sh '/kaniko/executor -f `pwd`/Dockerfile -c `pwd` --insecure --skip-tls-verify --cache=true --destination=docker.io/devsecopsandres/dsodemo'
            }
          }
        }
      }
    ...
    stage('Image Analysis') {
      parallel {
        stage('Image Linting') {
          steps {
            container('docker-tools') {
              sh 'dockle docker.io/devsecopsandres/dsodemo'
            }
          }
        }
        // stage('Image Scan') {
        //   steps {
        //     container('docker-tools') {
        //       sh 'trivy image --exit-code 1 devsecopsandres/dsodemo'
        //     }
        //   }
        // }
      }
